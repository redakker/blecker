...
void BlueTooth::onResult(BLEAdvertisedDevice advertisedDevice) {
    ...
    // logger << "Scanned device: " << advertisedDevice.getName().c_str() << " " << advertisedDevice.getAddress().toString().c_str();

    // Get RSSI and filter out very weak signals
    int rssiValue = advertisedDevice.getRSSI();
    if (rssiValue < -50) {
        // Ignore very weak signals to save CPU & network load
        return;
    }
    ...
    String deviceRSSI = String(rssiValue);  // store as string after filtering
    deviceMac.replace(":", "");
    ...
    if (!monitorObservedOnly) {
        if (newFound) {
            ...
            devices.add(dev);
            logger << "New device found. MAC: " << deviceMac << " RSSI: " << deviceRSSI;
            // Send an MQTT message about this device being at home
            handleDeviceChange(dev);
        }
    }
}
